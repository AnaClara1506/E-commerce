<?php
    ...
    if($_FILES['imagem']) 
    {
       ...
       if(move_uploaded_file($varImagemRecebida, $varArquivoPasta)){ //Move o arquivo para a pasta e retorna true se obtiver sucesso
        $varSQL = "UPDATE produto SET caminho_foto = :caminho_foto WHERE id_produto = :id_produto";
        $update = $conn->prepare($varSQL);
        $update->bindParam(':id_produto', $id);
        $update->bindParam(':caminho_foto', $varArquivoPasta); //Cria o parâmetro para o banco do arquivo salvo
        $update->execute();
       }
    } 
